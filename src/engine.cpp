...
Color ChessEngine::getSideToMove() const { return position.turn(); }

bool ChessEngine::isInCheck(Color side) const {
  return (side == WHITE) ? position.in_check<WHITE>()
                         : position.in_check<BLACK>();
}
...
std::string ChessEngine::moveToString(const Move &move) const {
  ...
  if (f == PR_KNIGHT || f == PC_KNIGHT)
    ss << "n";
  else if (f == PR_BISHOP || f == PC_BISHOP)
    ss << "b";
  else if (f == PR_ROOK || f == PC_ROOK)
    ss << "r";
  else if (f == PR_QUEEN || f == PC_QUEEN)
    ss << "q";
...
ChessEngine::runAnalysis(const std::vector<std::string> &positions,
...
void ChessEngine::printSearchStats() {
  ...
  if (searchStats.fail_high > 0)
    cutoff_rate =
        (double)searchStats.fail_high_first / searchStats.fail_high * 100.0;

  std::cout << "Search Stats:\n"
            << " - Nodes: " << searchStats.nodes << " (" << searchStats.qnodes
            << " qnodes)\n"
            << " - Avg branching factor: " << std::fixed << std::setprecision(2)
            << branching << "\n"
            << " - Hash hits: " << searchStats.hash_hits << " (" << hash_rate
            << "%)\n"
            << " - Fail-high first: " << searchStats.fail_high_first << "/"
            << searchStats.fail_high << " (" << cutoff_rate << "%)\n"
...
void ChessEngine::testPerft() {
  ...
  for (const PerftTest &test : tests) {
    ...
    std::cout << "FEN: " << test.fen << "\n"
              << "Depth: " << test.depth << "  Nodes: " << result
              << "  Expected: " << test.expected << "  Time: " << time_ms
              << "ms"
              << "  NPS: " << (uint64_t)nps << "  " << (ok ? "PASS" : "FAIL")
              << "\n"
              << "----------\n";
    if (ok)
      passed++;
...
MatchResult ChessEngine::selfPlayGames(int games, int depth,
                                       bool useTimeControl, int msPerMove,
                                       bool useOpeningBook) {
  MatchResult result;
  ...
  for (int i = 0; i < games; i++) {
    ...
    while (winner == -1 && half_moves < 200) {
      ...
      if (useOpeningBook && half_moves < 20) {
        ...
        if (move == Move()) {
          move = useTimeControl ? getBestMoveWithTime(msPerMove)
                                : getBestMove(depth);
        }
      } else {
        move = useTimeControl ? getBestMoveWithTime(msPerMove)
      ...
      if (isCheckmate()) {
        winner = (getSideToMove() == WHITE) ? 2 : 1;
      } else if (isStalemate() || isDrawByInsufficientMaterial() ||
                 isRepetition() || half_moves >= 200) {
        winner = 0;
      }
    }
...
void ChessEngine::uciLoop() {
  ...
  while (std::getline(std::cin, line)) {
    ...
    if (token == "quit") {
      ...
      Move best_move =
          (movetime > 0) ? getBestMoveWithTime(movetime) : getBestMove(depth);
...
bool ChessEngine::isThreefoldRepetition() const {
  if (repetition_history.size() < 5)
    return false;
  uint64_t cur = position.get_hash();
  int count = 0;
  int limit =
      std::min((int)repetition_history.size(), position.halfmove_clock() + 1);
  for (int i = 0; i < limit; i++) {
    size_t idx = repetition_history.size() - 1 - i;
    if (repetition_history[idx] == cur) {
      count++;
      if (count >= 3)
        return true;
    }
...
ChessEngine::GameResult ChessEngine::getGameResult() {
  if (isCheckmate()) {
    return (getSideToMove() == WHITE) ? GameResult::BlackWinsCheckmate
...
std::string ChessEngine::gameResultToString(GameResult r) {
  switch (r) {
